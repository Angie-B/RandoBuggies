...
# write.csv(aq.detect,"in.HILIC.aq.AOA.SUP05.csv")
```

```{r combine}
measured.tqs <- aq.detect %>% filter(percent.aq.blksub>0.15)%>%
     filter(percent.aq.raw>0)
...
#                            "BackFilled",Notes))
new.data.2 <- aq.data.sub    
```

##Calculate mean values for each IS
##USING JUST AREAS NOT BLANK SUB
```{r ISmeans}
